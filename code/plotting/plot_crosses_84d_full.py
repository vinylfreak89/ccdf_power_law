...
# Load SPX - full dataset from 1920
print("Loading SPX data...")
df = load_asset('_spx_d.csv')  # No min_date = full dataset
...
# Top: SPX price with cross-under markers (LOG SCALE)
...
ax1.set_title(f'SPX (1920-2025) with Deviation Crosses Under {baseline_window}d Baseline (Red lines)', fontsize=13, fontweight='bold')
...
for idx in crosses_under:
    ax2.plot(df.iloc[idx]['Date'], df.iloc[idx]['Mean_Deviation'], 'ro', markersize=5)
...
plt.tight_layout()
plt.savefig('deviation_crosses_84d_full.png', dpi=300, bbox_inches='tight')
print("✓ Saved deviation_crosses_84d_full.png")

print(f"\nDataset: {df.iloc[0]['Date']} to {df.iloc[-1]['Date']}")
