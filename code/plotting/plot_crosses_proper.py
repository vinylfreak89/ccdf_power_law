...
# Calculate baseline - use min_periods to handle NaN better
baseline_window = 252
df['Baseline'] = df['Mean_Deviation'].rolling(window=baseline_window, min_periods=1).mean()

print(f"Mean_Deviation NaN count: {df['Mean_Deviation'].isna().sum()}")
print(f"Baseline NaN count: {df['Baseline'].isna().sum()}")

# Find crosses back under baseline (only where both values exist)
crosses_under = []
...
ax1.set_title('SPX with Deviation Crosses Under Baseline (Red lines)', fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('deviation_crosses_under.png', dpi=300, bbox_inches='tight')
print("✓ Saved deviation_crosses_under.png")

# Print first 20 cross dates
print("\nFirst 20 cross-under dates:")
for idx in crosses_under[:20]:
    print(f"  {df.iloc[idx]['Date'].strftime('%Y-%m-%d')}: Dev={df.iloc[idx]['Mean_Deviation']:.6f}, Base={df.iloc[idx]['Baseline']:.6f}")
