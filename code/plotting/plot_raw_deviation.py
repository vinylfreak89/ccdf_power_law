...
ax1.set_title('SPX Price (1920-2025)', fontsize=13, fontweight='bold')
ax1.grid(True, alpha=0.3)

# Bottom: Raw mean deviation
...
plt.tight_layout()
plt.savefig('raw_mean_deviation_full.png', dpi=300, bbox_inches='tight')
print("✓ Saved raw_mean_deviation_full.png")

# Print basic stats
print(f"\nBasic statistics:")
print(f"  Mean: {df['Mean_Deviation'].mean():.6f}")
print(f"  Std: {df['Mean_Deviation'].std():.6f}")
print(f"  Min: {df['Mean_Deviation'].min():.6f}")
print(f"  Max: {df['Mean_Deviation'].max():.6f}")
print(f"  % Above zero: {(df['Mean_Deviation'] > 0).sum() / df['Mean_Deviation'].notna().sum() * 100:.1f}%")
